...
class ImageFolderOCRSegment(data.Dataset):
	...
	def __getitem__(self, index):
		...
		if (self.mode == 'train') and p_transform <= self.augmentation_prob and False:
			RotationDegree = random.randint(0,3)
			RotationDegree = self.RotationDegree[RotationDegree]
			if (RotationDegree == 90) or (RotationDegree == 270):
				aspect_ratio = 1/aspect_ratio
			...
			ShiftRange_lower = image.size[1] - random.randint(0,20)
			image = image.crop(box=(ShiftRange_left,ShiftRange_upper,ShiftRange_right,ShiftRange_lower))
			GT = [_GT.crop(box=(ShiftRange_left,ShiftRange_upper,ShiftRange_right,ShiftRange_lower)) for _GT in GT]
		...
		Norm_ = T.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
		image = Norm_(image)
		return image, torch.cat(GT), self.img_paths[index]
...
def get_loader_ocr(image_path, image_size, batch_size, num_workers=2, mode='train',augmentation_prob=0.4, num_ch=14):
	...
	if mode == 'test':
		dataset_test = ImageFolderOCRSegment(root = image_path, image_size =image_size, mode='test',augmentation_prob=-1, num_ch=num_ch)
		image_path = image_path.replace('/test', '/train')
		dataset_train = ImageFolderOCRSegment(root = image_path, image_size =image_size, mode='test',augmentation_prob=-1, num_ch=num_ch)
		
		dataset = torch.utils.data.ConcatDataset([dataset_train, dataset_test])
	else:
		dataset = ImageFolderOCRSegment(root = image_path, image_size =image_size, mode=mode,augmentation_prob=augmentation_prob, num_ch=num_ch)
	data_loader = data.DataLoader(dataset=dataset,
								  batch_size=batch_size if mode=='train' else 1,
								  shuffle=True,
								  num_workers=num_workers)
